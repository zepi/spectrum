...
QUnit.test( "Default Color Is Set By Input Value", function(assert) {
  ...
  assert.equal( noValue.spectrum("get"), null, "Defaults to null with no value set");
...
QUnit.test("Toggle Picker Area button works as expected", function(assert) {
  var div = $("<div style='height:20px; width:150px'>").appendTo('body').show(),
  ...
  assert.expect(Math.round(picker.offset().left) < Math.round(palette.offset().left), "The picker area is next to the palette.");
